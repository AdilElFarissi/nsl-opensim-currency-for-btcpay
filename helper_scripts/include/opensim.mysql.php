...
function  opensim_get_avatar_session($uuid, &$db=null)
{
    ...
    // Standalone
        ...
        $sql = "SELECT SessionID,SecureSessionID FROM MoneyPresence WHERE UserID='".$uuid."'";
        $db->query($sql);
        list($SessionID,$SecureSessionID) = $db->next_record();
        
        $avssn['regionID']  = $RegionID;
        $avssn['sessionID'] = $SessionID != null ? $SessionID : UUID_ZERO;
        $avssn['secureID']  = $SecureSessionID != null ? $SecureSessionID : UUID_ZERO;
    }
...
function  opensim_get_object_name($uuid, &$db=null)
{
    ...
    if ($objName=='' and $db->exist_table('transactions')) { 
        $query_str = "SELECT objectName FROM transactions WHERE objectUUID='$uuid'";
        $db->query($query_str);
        if ($db->Errno==0) list($objName) = $db->next_record();
    }
...
$TransactionType['5013']  = 'WithrawMoney';
...
function  add_new_btcpay_invoice($agentID, $invoiceID, $invoiceStatus, $invoiceAmount, &$db=null)
{
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
    $Id = make_random_guid();
   
        $sql = "INSERT INTO Invoices (UUID,UserID, InvoiceID, InvoiceStatus, InvoiceAmount) VALUES ('$Id', '$agentID', '$invoiceID', '$invoiceStatus', '$invoiceAmount')";
        $db->query($sql);
        if ($db->Errno==0) return true;
        return false;
}

function  add_new_btcpay_pull_payment($agentID, $pullPaymentID, $payoutID, $pullPaymentStatus, $payoutAmount, $localAmount, &$db=null)
{
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
    $Id = make_random_guid();
   
        $sql = "INSERT INTO Withdrawals (UUID,UserID, PullPaymentID, PayoutID, PullPaymentStatus, PayoutAmount, LocalAmount) VALUES ('$Id', '$agentID', '$pullPaymentID', '$payoutID', '$pullPaymentStatus', '$payoutAmount', '$localAmount')";
        $db->query($sql);
        if ($db->Errno==0) return true;
        return false;
}

function  get_btcpay_pull_payment_data($pullPaymentID, $payoutID){
    if (!is_object($db)) $db = opensim_new_db();

    $sql = "SELECT UserID,PullPaymentStatus,PayoutAmount,LocalAmount FROM Withdrawals WHERE PullPaymentID='$pullPaymentID' AND PayoutID='$payoutID'";
    $db->query($sql);
    list($agentID,$status,$fiatAmount,$localAmount) = $db->next_record();
    ...
    if ($db->Errno==0) return $ret;
        return null;
}

function has_pending_btcpay_invoice($agentID, &$db=null){
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
   
        $sql = "SELECT InvoiceID FROM Invoices WHERE UserID='$agentID' AND InvoiceStatus='new'";
        $db->query($sql);
        list($invoiceID) = $db->next_record();
        if ($db->Errno==0 && $invoiceID != null) return $invoiceID;
        return null;
}

function has_pending_btcpay_pull_payment($agentID, &$db=null){
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
   
        $sql = "SELECT PullPaymentID FROM Withdrawals WHERE UserID='$agentID' AND PullPaymentStatus='new' OR PullPaymentStatus='AwaitingApproval'";
        $db->query($sql);
        list($pullPaymentID) = $db->next_record();
        if ($db->Errno==0 && $pullPaymentID != null) return $pullPaymentID;
        return null;
}

function update_btcpay_invoice_status($agentID, $invoiceID, $invoiceStatus, &$db=null)
{
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
   
        $sql = "UPDATE Invoices SET InvoiceStatus='$invoiceStatus' WHERE UserID='$agentID' AND InvoiceID='$invoiceID'";
        $db->query($sql);
        if ($db->Errno==0) return true;
        return false;
}

function update_btcpay_payout_status($agentID, $pullPaymentID, $payoutID, $pullPaymentStatus, &$db=null)
{
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
   
        $sql = "UPDATE Withdrawals SET PullPaymentStatus='$pullPaymentStatus' WHERE UserID='$agentID' AND PullPaymentID='$pullPaymentID' AND PayoutID='$payoutID'";
        $db->query($sql);
        if ($db->Errno==0) return true;
        return false;
}

function remove_btcpay_invoice($agentID, $invoiceID, &$db=null)
{
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
   
        $sql = "DELETE FROM Invoices WHERE UserID='$agentID' AND InvoiceID='$invoiceID'";
        $db->query($sql);
        if ($db->Errno==0) return true;
        return false;
}

function remove_btcpay_pull_payment($agentID, $pullPaymentID, $payoutID, &$db=null)
{
    if (!isGUID($agentID)) return false;
    if (!is_object($db)) $db = opensim_new_db();
   
        $sql = "DELETE FROM Withdrawals WHERE UserID='$agentID' AND PullPaymentID='$pullPaymentID' AND PayoutID='$payoutID'";
        $db->query($sql);
        if ($db->Errno==0) return true;
        return false;
}
...
function  opensim_check_secure_session($uuid, $regionid, $secure, &$db=null)
{
    ...
    else if(!$db->exist_table('Presence') && $db->exist_table('MoneyPresence')){
        $sql = "SELECT UserID FROM MoneyPresence WHERE UserID='".$uuid."' AND SecureSessionID='".$secure."'";
    }
    // Standalone
    else {
